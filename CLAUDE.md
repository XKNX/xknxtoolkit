...
- `xknx-keys` — parses/serializes KNX keyring XML, depends on models
- `apps/knx-gui` — node-based GUI app; consumes all packages; see `apps/knx-gui/CLAUDE.md`

**Import paths** use the `xknx` namespace package (src layout):
- `from xknx.models import ...`
- `from xknx.product import ...`
- `from xknx.project import ...`
- `from xknx.keys import ...`
...
1. Create `packages/<name>/` with `pyproject.toml` (`name = "xknx-<name>"`), `src/xknx/<name>/__init__.py`, `src/xknx/<name>/py.typed`, and `tests/`
