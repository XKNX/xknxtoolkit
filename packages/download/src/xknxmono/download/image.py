...
def build_image(
    ...
    *,
    ...
    segments = [
        ...
        if data and segment_id in base_addresses
    ]
    ...
    if com_object_table is not None and com_object_table.code_segment is not None:
        ...
        if seed is not None and base is not None:
            data, mask = _build_com_object_table(
                seed[0], ui, linked_numbers, set(number_of.values())
            )
...
def _group_communication_relative_segments(
    ...
    descriptors = _resolved_group_object_descriptors(ui, linked_numbers)
...
def _build_com_object_table(
    ...
    Follows the Group Object Table definition (3/5/1 Resources, 4.18): a com object visible in the current
    configuration gets its full flag byte and size recomputed (communication bit
    set only when the object is linked); every other defined object keeps the
