...
class LoadProcedureRunner:
    ...
    async def _execute(self, control: object) -> None:
        ...
        if isinstance(control, LdCtrlCompareMem):
            _require_standard(control.address_space)
            await self._compare_mem(control.address, control.inline_data)
            return
        ...
        if isinstance(control, LdCtrlCompareRelMem):
            ...
            await self._compare_mem(base + control.offset, control.inline_data)
            return
    ...
    async def _compare_mem(self, address: int, expected: bytes) -> None:
        """Read memory and compare it against expected data."""
        programmer = await self._bus()
    ...
    async def _compare_prop(self, control: LdCtrlCompareProp) -> None:
        ...
        if mask:
            ...
            matches = read_back[:length] == expected[:length]
        if not matches:
            raise VerificationError(
...
def _require_standard(address_space: LdCtrlMemAddrSpace) -> None:
    """Reject memory operations outside the standard address space."""
    if address_space != LdCtrlMemAddrSpace.STANDARD:
        message = (
            f"memory address space {address_space.value!r} is not implemented; only "
            f"the standard address space is handled so far (KNX Standard v3.0.0, 3/5/1 "
            f"Resources). Please file a bug report with the product data so this "
            f"address space can be added."
        )
        logger.warning("unsupported load control: %s", message)
