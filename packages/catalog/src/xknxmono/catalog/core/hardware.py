...
def list_hardware(
  ...
  if filters.no_download_without_plugin is not None:
    q = q.where(Hardware.no_download_without_plugin == filters.no_download_without_plugin)
  if filters.search:
...
def get_application_detail(
  ...
  Returns:
    A :class:`~xknxmono.product.application.DeviceApplication` instance with
  ...
  with archive:
    ...
    return next((a for a in apps if a.application_id == program.application_id), None)
