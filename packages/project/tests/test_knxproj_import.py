...
# Optional smoke test against a real project. Provide your own ``.knxproj`` via the
# ``XKNX_TEST_KNXPROJ`` environment variable (no sample is bundled).
_REAL_ENV = os.environ.get("XKNX_TEST_KNXPROJ")
_REAL = Path(_REAL_ENV) if _REAL_ENV else None


@pytest.mark.skipif(
    _REAL is None or not _REAL.exists(),
    reason="set XKNX_TEST_KNXPROJ to a .knxproj to run this test",
)
