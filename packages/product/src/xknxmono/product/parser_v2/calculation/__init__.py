...
def evaluate_lr(calc: ParameterCalculation, l_values: dict[str, str], script: str | None = None) -> dict[str, str]:
    ...
    if calc.lrtransformation_func:
        assert script is not None, f"calc {calc.id!r} has LRTransformationFunc but no Script block"
        return eval_named_func(script, calc.lrtransformation_func, calc.lrtransformation_parameters, l_values, r_names)
    if calc.lrtransformation:
        return eval_inline(calc.lrtransformation, l_values, r_names)
    return {}


def evaluate_rl(calc: ParameterCalculation, r_values: dict[str, str], script: str | None = None) -> dict[str, str]:
    ...
    if calc.rltransformation_func:
        assert script is not None, f"calc {calc.id!r} has RLTransformationFunc but no Script block"
        return eval_named_func(script, calc.rltransformation_func, calc.rltransformation_parameters, r_values, l_names)
