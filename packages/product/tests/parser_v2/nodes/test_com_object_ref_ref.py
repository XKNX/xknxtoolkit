...
class TestComObjectRefRefNodeBasic:
    def test_eval_returns_ui_com_object(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(_ELEM, _cor(), _co())
        result = node.eval(EvalContext(GlobalState(), idx=idx))
        assert len(result) == 1
        assert isinstance(result[0], UiComObject)

    def test_ref_id_is_passed_through(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(_ELEM, _cor(), _co())
        co = node.eval(EvalContext(GlobalState(), idx=idx))[0]
        assert isinstance(co, UiComObject)
        assert co.ref_id == _REF_ID

    def test_number_from_base(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(_ELEM, _cor(), _co(number=42))
        co = node.eval(EvalContext(GlobalState(), idx=idx))[0]
        assert isinstance(co, UiComObject)
        assert co.number == 42

    def test_dpt_codes_parsed_from_base(self, idx: ApplicationIndexer):
    ...
    def test_dpt_codes_ref_overrides_base(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(
    ...
    def test_dpt_codes_skips_malformed_token(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(
    ...
    def test_dpt_codes_skips_non_numeric_major(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(
...
class TestFlags:
    def test_flags_from_base(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(
            _ELEM,
            _cor(),
            _co(
    ...
    def test_ref_flag_overrides_base(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(
            _ELEM,
    ...
    def test_locked_flags_from_base_only(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(
            _ELEM,
            _cor(),
            _co(
    ...
    def test_none_base_falls_back_to_defaults(self, idx: ApplicationIndexer):
...
class TestName:
    def test_name_from_base_text(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(
    ...
    def test_ref_text_overrides_base_text(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(_ELEM, _cor(text="Override"), _co(text="Base"))
        co = node.eval(EvalContext(GlobalState(), idx=idx))[0]
        assert isinstance(co, UiComObject)
        assert co.name == "Override"

    def test_name_with_placeholder_filled_from_param(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(
            _ELEM,
        ...
        co = node.eval(EvalContext(state, idx=idx))[0]
        assert isinstance(co, UiComObject)
        assert co.name == "Channel A Switch"

    def test_name_placeholder_empty_when_param_missing(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(
            _ELEM,
            _cor(text="{{0}} Switch", text_parameter_ref_id=_TEXT_PARAM_REF),
            _co(),
...
class TestActiveTracking:
    def test_marks_local_ref_active(self, idx: ApplicationIndexer):
        node = ComObjectRefRefNode(_ELEM, _cor(), _co())
        state = GlobalState()
