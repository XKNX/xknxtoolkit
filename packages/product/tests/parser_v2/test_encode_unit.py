...
def test_collect_param_active_ref_is_written() -> None:
    ...
    app, idx = _app([p], parameter_refs=[ParameterRef(id="PR1", ref_id="P1")])
    state = GlobalState()
    state.mark_active_param("PR1", "P1")  # what a real ParameterRefRef would mark
    w = collect_writes(app, idx, {}, state)
    assert len(w.mem) == 1
...
def test_collect_param_inactive_in_one_module_instance_but_active_in_another() -> None:
    ...
    active_instance.mark_active_param("PR1", "MP1")
...
def test_module_instance_overrides_resolve_to_parameter_via_ref() -> None:
    ...
    ms.mark_active_param("PR1", "MP1")  # what a real ParameterRefRef would mark
    w = collect_writes(app, idx, {}, state)
    assert w.mem[0].value == "42"
