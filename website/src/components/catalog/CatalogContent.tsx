...
interface Manufacturer { id: string; name: string | null }
  ...
  const columns = useMemo(() => [
    columnHelper.accessor(h => h.name ?? '—', {
      ...
      cell: info => {
        ...
        return (
          <div>
            <div className="font-medium text-sm text-fd-foreground truncate">{info.getValue()}</div>
            {h.description && <div className="text-xs text-fd-muted-foreground truncate mt-0.5">{h.description}</div>}
          </div>
        )
      },
    }),
    columnHelper.accessor('manufacturer_id', {
      ...
      cell: info => <span className="text-xs text-fd-muted-foreground">{mfrMap[info.getValue()] ?? '—'}</span>,
    }),
    columnHelper.accessor('order_number', {
      header: 'Order #',
      cell: info => <span className="text-xs text-fd-muted-foreground">{info.getValue() ?? '—'}</span>,
    }),
      ...
      cell: ({ row: { original: h } }) => {
        ...
        return (
          <div className="flex gap-1">
            {mts.map(mt => (
              <span key={mt} className="text-xs px-1.5 py-0.5 rounded bg-fd-secondary text-fd-secondary-foreground">
                {mt}
              </span>
            ))}
          </div>
      ...
      header: () => (
        <div className="flex items-center gap-3 whitespace-nowrap">
          <label className="flex items-center gap-1 text-xs font-medium text-fd-foreground cursor-pointer select-none">
            <input type="checkbox" className="accent-fd-primary" checked={secureOnly} onChange={e => push({ secure: e.target.checked ? 'true' : null })} />
            Secure
          </label>
          <label className="flex items-center gap-1 text-xs font-medium text-fd-foreground cursor-pointer select-none">
            <input type="checkbox" className="accent-fd-primary" checked={railMounted} onChange={e => push({ rail: e.target.checked ? 'true' : null })} />
            Rail
          </label>
        </div>
      ),
      cell: ({ row: { original: h } }) => {
        ...
        return (
          <div className="flex gap-1 flex-wrap">
            {isSecure && <span className="text-xs px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-600 dark:text-blue-400">Secure</span>}
            {h.is_rail_mounted && <span className="text-xs px-1.5 py-0.5 rounded bg-fd-secondary text-fd-secondary-foreground">DIN Rail</span>}
          </div>
        )
      },
    }),
        ...
        <table className="w-full text-sm table-fixed">
            ...
            {table.getHeaderGroups().map(hg => (
              <tr key={hg.id}>
                {hg.headers.map(header => (
                  <th key={header.id} className="px-3 py-2.5 text-left font-medium text-fd-foreground align-top">
          ...
          <tbody>
            {table.getRowCount() === 0 && !loading && (
              <tr>
                <td colSpan={columns.length} className="px-4 py-6 text-sm text-fd-muted-foreground text-center">
                  No products found.
                </td>
              </tr>
            )}
