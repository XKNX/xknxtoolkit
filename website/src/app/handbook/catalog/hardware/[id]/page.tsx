...
function Badge({
  ...
  color?: "secondary" | "blue" | "green" | "amber";
}) {
  const styles = {
  ...
  return (
    <DocsPage footer={{ enabled: false }} breadcrumb={{ enabled: false }} toc={toc}>
      ...
      <div className="text-fd-muted-foreground mt-1">
        {manufacturer.name ?? hardware.manufacturer_id}
      </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-x-8 gap-y-4">
        ...
        <Field
          label="Bus current"
          value={
            hardware.bus_current != null && hardware.bus_current > 0
              ? `${hardware.bus_current} mA`
              : null
          }
        />
        <Field
          label="Width"
          value={
            hardware.width_mm != null && hardware.width_mm > 0
        ...
        <Field
          label="Has application program"
          value={
            hardware.has_application_program != null
        ...
        />
        <Field
          label="No download without plugin"
          value={hardware.no_download_without_plugin ? "Yes" : null}
        />
      </div>
            ...
            {hardware.programs.map((prog) => (
              <div key={prog.id} className="rounded-lg border border-fd-border p-4">
                <div className="flex items-start justify-between gap-4 mb-3">
                  ...
                  {prog.application && (
                    <Link
                      href={`/handbook/catalog/hardware/${encodeURIComponent(id)}/programs/${encodeURIComponent(prog.id)}/application`}
