  ...
  if (param.param_type.kind === 'enum' && param.param_type.options.length > 0) {
    ...
    return (
      <span className="text-xs">
        ...
        <span className="text-fd-muted-foreground/50 ml-1">({param.value})</span>
      </span>
    )
  }
  return <span className="font-mono text-xs">{param.value}</span>
...
export default async function ApplicationPage({
  ...
  const totalSegments = (app.code?.absolute_segments.length ?? 0) + (app.code?.relative_segments.length ?? 0)
  ...
  const toc = [
    ...
    ...(app.com_objects.length > 0
      ? [{ title: `Communication Objects (${app.com_objects.length})`, url: '#com-objects', depth: 2 }]
      : []),
    ...(app.parameters.length > 0
      ? [{ title: `Parameters (${app.parameters.length})`, url: '#parameters', depth: 2 }]
      : []),
  ]
  ...
  return (
    <DocsPage footer={{ enabled: false }} breadcrumb={{ enabled: false }} toc={toc}>
      ...
      <div className="text-fd-muted-foreground mt-1 text-sm">{app.manufacturer_id}</div>
      ...
      {app.code && totalSegments > 0 && (
        <div id="code" className="mt-8">
          ...
          <div className="space-y-6">
            {[...app.code.absolute_segments].sort((a, b) => a.address - b.address).map(seg => (
              <div key={seg.id}>
            ...
            {[...app.code.relative_segments].sort((a, b) => a.offset - b.offset).map(seg => (
              <div key={seg.id}>
                <div className="flex items-baseline gap-3 mb-2">
                  <span className="text-sm font-medium text-fd-foreground">
                    ...
                    <td className="py-2 pr-4">
                      <div className="flex flex-wrap gap-1">
                        {co.dpt_codes.length > 0
                          ? co.dpt_codes.map((d, i) => <DptBadge key={`${d}-${i}`} code={d} />)
                          : <span className="text-xs text-fd-muted-foreground/40">—</span>}
                      </div>
                    </td>
                    <td className="py-2">
                      <div className="flex gap-0.5">
                        <FlagCell active={co.flags.communication} label="Communication" abbr="C" />
                        <FlagCell active={co.flags.read} label="Read" abbr="R" />
                        <FlagCell active={co.flags.write} label="Write" abbr="W" />
                        <FlagCell active={co.flags.transmit} label="Transmit" abbr="T" />
                        <FlagCell active={co.flags.update} label="Update" abbr="U" />
                        <FlagCell active={co.flags.read_on_init} label="Read on Init" abbr="I" />
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-fd-border text-left">
                  ...
                  <th className="pb-2 pr-4 text-xs font-medium text-fd-muted-foreground">Value</th>
                  <th className="pb-2 text-xs font-medium text-fd-muted-foreground">Type</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-fd-border/50">
                {app.parameters.map(p => (
                  <tr key={p.id} className="hover:bg-fd-muted/30 transition-colors">
                    <td className="py-2 pr-4 text-fd-foreground">{p.name || <span className="text-fd-muted-foreground/40">—</span>}</td>
                    <td className="py-2 pr-4 text-fd-muted-foreground text-xs">{p.text || <span className="text-fd-muted-foreground/40">—</span>}</td>
                    <td className="py-2 pr-4">
                      <ParamValueDisplay param={p} />
                    </td>
                    <td className="py-2">
                      {p.param_type ? <ParamTypeDisplay pt={p.param_type} /> : <span className="text-xs text-fd-muted-foreground/40">—</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
