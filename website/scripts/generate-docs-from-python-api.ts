/*
  fork of fumadocs-python /convert.ts
  https://github.com/fuma-nama/fumadocs/tree/dev/packages/python
*/
...
export function convert(mod: ModuleInterface, options: ConvertOptions = {}): OutputFile[] {
  ...
  if (Object.keys(mod.modules).length > 0) {
    content.push(heading(2, "Modules"));
    const lines: string[] = [];
    ...
    content.push(element("Cards", undefined, lines.join("\n")));
  }

  if (Object.keys(mod.classes).length > 0) {
    content.push(heading(2, "Classes"));
    const lines: string[] = [];
