...
class KeyringPanel:
    ...
    def render(self) -> None:
        self._poll_dialog()
        if self._service.keyring is None:
            self._render_import()
            return
    ...
    def _render_import(self) -> None:
        if self._pending_path is None:
            imgui.text_disabled(S.KEYRING_EMPTY)
            if imgui.button(S.KEYRING_IMPORT):
                self._dialog = pfd.open_file(
                    S.KEYRING_IMPORT,
                    "",
                    [S.KEYRING_FILTER, "*.knxkeys", S.ALL_FILES, "*"],
                )
            return
        imgui.text(Path(self._pending_path).name)
